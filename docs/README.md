...
| 4   | **[CI & Commits](./04-ci-and-commits.md)** 🚦                       | How commit messages are enforced and what the pipeline checks on every push.                                                           |
...
> 💡 These are exactly the checks CI runs, in order: `lint` → `test` →
> `test:e2e` → `build`. Run them locally before you push — see
> [doc 4](./04-ci-and-commits.md).
